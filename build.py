...
def build(*test_flags):
    ...
    built_file = build_dir() / "rusty-kern.efi"
...
def find_ovmf():
    ...
    ovmf_dir = WORKSPACE_DIR / "rusty-kern"
    if check_ovmf_dir(ovmf_dir):
        return ovmf_dir
...
def run_qemu():
    ...
    qemu_flags.extend(
        [
            # Set up OVMF.
            "-drive",
            f"if=pflash,format=raw,file={ovmf_code},readonly=on",
            "-drive",
            f"if=pflash,format=raw,file={ovmf_vars},readonly={ovmf_vars_readonly}",
    ...
    try:
        ...
        with open(monitor_input_path, mode="w") as monitor_input, open(
            monitor_output_path, mode="r"
        ) as monitor_output:
...
def main():
    ...
    parser.add_argument(
        "--target",
        help="target to build for (default: %(default)s)",
        type=str,
    ...
    parser.add_argument(
        "--headless", help="run QEMU without a GUI", action="store_true"
    )

    parser.add_argument("--release", help="build in release mode", action="store_true")

    parser.add_argument(
        "--ci", help="disables some tests which currently break CI", action="store_true"
    )
